...
import com.stytch.sdk.ui.b2c.screens.ReturningUserScreenViewModel
...
internal class ReturningUserScreenViewModelTest {
    ...
    @Test
    fun `initializeState correctly initializes the state`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `onPasswordChanged updates the state as expected`() =
        runTest(dispatcher) {
    ...
    fun `authenticate emits correct event on reset_password success`() =
        runTest(dispatcher) {
            ...
            } returns mockk<StytchResult.Success<BasicData>>()
            val eventFlow =
                async {
                    viewModel.eventFlow.first()
                }
    ...
    @Test
    fun `authenticate updates state on non reset_password StytchAPIError`() =
        runTest(dispatcher) {
            val result: StytchResult.Error =
                mockk(relaxed = true) {
                    every { exception } returns
    ...
    @Test
    fun `authenticate updates state on non StytchAPIError`() =
        runTest(dispatcher) {
            val result: StytchResult.Error =
                mockk(relaxed = true) {
                    every { exception } returns
