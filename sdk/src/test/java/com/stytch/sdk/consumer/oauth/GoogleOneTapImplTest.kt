...
internal class GoogleOneTapImplTest {
    ...
    @Test
    fun `GoogleOneTapImpl start returns false if nonce fails to generate`() =
        runTest {
    ...
    @Test
    fun `GoogleOneTapImpl start returns false if oneTapClient fails to initialize`() =
        runTest {
            every { mockGoogleOneTapProvider.getSignInClient(any()) } returns
                mockk {
    ...
    @Test
    fun `GoogleOneTapImpl start returns false if getSignInRequest fails`() =
        runTest {
    ...
    @Test
    fun `GoogleOneTapImpl start returns false if oneTapClient calls FailureListener`() =
        runTest {
            val fail = slot<OnFailureListener>()
            val mockedTask: Task<BeginSignInResult> =
                mockk(relaxed = true) {
    ...
    @Test
    fun `GoogleOneTapImpl start returns false if startIntentSenderForResult throws`() =
        runTest {
    ...
    @Test
    fun `GoogleOneTapImpl start returns true if startIntentSenderForResult succeeds`() =
        runTest {
            val success = slot<OnSuccessListener<BeginSignInResult>>()
            val mockedTask: Task<BeginSignInResult> =
                mockk(relaxed = true) {
    ...
    @Test
    fun `GoogleOneTapImpl authenticate returns error if nonce is missing`() =
        runTest {
    ...
    @Test
    fun `GoogleOneTapImpl authenticate returns error if onetap client is not initialized`() =
        runTest {
            impl.nonce = "nonce"
            val result = impl.authenticate(mockk())
            require(result is StytchResult.Error)
    ...
    @Test
    fun `GoogleOneTapImpl authenticate returns error if ApiException is thrown`() =
        runTest {
            impl.nonce = "nonce"
            impl.oneTapClient =
                mockk {
    ...
    @Test
    fun `GoogleOneTapImpl authenticate returns correct error if idToken is missing`() =
        runTest {
            impl.nonce = "nonce"
            impl.oneTapClient =
                mockk {
                    every { getSignInCredentialFromIntent(any()) } returns
                        mockk {
    ...
    @Test
    fun `GoogleOneTapImpl authenticate returns error if authenticateWithGoogleIdToken fails`() =
        runTest {
            impl.nonce = "nonce"
            impl.oneTapClient =
                mockk {
            ...
            coEvery { mockApi.authenticateWithGoogleIdToken(any(), any(), any()) } returns
                StytchResult.Error(
    ...
    @Test
    fun `GoogleOneTapImpl authenticate returns success if authenticateWithGoogleIdToken succeeds`() =
        runTest {
            impl.nonce = "nonce"
            impl.oneTapClient =
                mockk {
            ...
            val mockResponse: StytchResult.Success<NativeOAuthData> =
                mockk {
    ...
    fun `GoogleOneTapImpl signout delegates to onetap client`() {
        val mockSignInClient: SignInClient =
            mockk {
