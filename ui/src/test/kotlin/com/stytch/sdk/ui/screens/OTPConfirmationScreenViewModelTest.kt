...
internal class OTPConfirmationScreenViewModelTest {
    ...
    @Test
    fun `setInitialState for Email sets the state and kicks off the countdown timer`() = runTest(dispatcher) {
        every { DateUtils.formatElapsedTime(any()) } returns "Formatted DateTime String"
        val emailDetails = OTPDetails.EmailOTP(
    ...
    @Test
    fun `setInitialState for SMS sets the state and kicks off the countdown timer`() = runTest(dispatcher) {
        every { DateUtils.formatElapsedTime(any()) } returns "Formatted DateTime String"
        val smsDetails = OTPDetails.SmsOTP(
    ...
    @Test
    fun `setInitialState for WhatsApp sets the state and kicks off the countdown timer`() = runTest(dispatcher) {
        val whatsappDetails = OTPDetails.WhatsAppOTP(
    ...
    @Test
    fun `authenticate updates state and emits event on success`() = runTest(dispatcher) {
    ...
    @Test
    fun `authenticate updates state on failure`() = runTest(dispatcher) {
    ...
    @Test
    fun `resendOTP with email delegates to the appropriate StytchClient method`() = runTest(dispatcher) {
    ...
    @Test
    fun `resendOTP with sms delegates to the appropriate StytchClient method`() = runTest(dispatcher) {
    ...
    @Test
    fun `resendOTP with whatsapp delegates to the appropriate StytchClient method`() = runTest(dispatcher) {
    ...
    @Test
    fun `resendOTP updates everything as expected on success`() = runTest(dispatcher) {
    ...
    @Test
    fun `resendOTP updates everything as expected on failure`() = runTest(dispatcher) {
        val result: StytchResult.Error = mockk(relaxed = true) {
            every { exception } returns mockk<StytchInternalError> {
                every { message } returns "Something bad happened internally"
            }
    ...
    @Test
    fun `sendResetPasswordEmail updates error if no email is passed`() = runTest(dispatcher) {
